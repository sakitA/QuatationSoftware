        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(Keys.ITEM_LIST_QUERY);) {
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    item.setItemCodeL(resultSet.getString("item_code_l"));
                    item.setItemCodeM(resultSet.getString("item_code_m"));
                    item.setItemCodeS(resultSet.getString("item_code_s"));
                    item.setItemDescriptionS(resultSet.getString("item_description_s"));
                    item.setItemDescriptionL(resultSet.getString("item_description_l"));
    ...
    public void save(List<Item> listSave, int quotationId, int bathroomId) throws Exception {        
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            for (Item item : listSave) {
                ...
                preparedStatement.setString(4, item.getItemCodeL());
                preparedStatement.setString(5, item.getItemCodeM());
                preparedStatement.setString(6, item.getItemCodeS());
                preparedStatement.setString(7, item.getItemDescriptionS());
                preparedStatement.setString(8, item.getItemDescriptionL());
